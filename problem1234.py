...
class Person:
	...
	def __eq__(self, other):
		return self.name == (other and other.name)
...
# -------------- Problem 3 get mothers with most daughters ------------------ #
	def get_mothers_with_most_daughters(self):
		# Find Max daughters
		max_daughters = 0
		for person in self.relation_map:
			person_obj = self.relation_map[person]['self']
			if person_obj.name == 'Anga':
				continue

			if person_obj.gender == 'F':
				daughters = len(self.get_daughters(person_obj))
				if daughters and daughters >= max_daughters:
					max_daughters = daughters

		# Find mother with max daughters
		max_mothers = []
		for person in self.relation_map:
			person_obj = self.relation_map[person]['self']
			if person_obj.name == 'Anga':
			...
			if person_obj.gender == 'F':
				...
				if daughters == max_daughters:
					max_mothers.append(person_obj)

		return [m.name for m in max_mothers]
# -------------- Problem 3 get mothers with most daughters ------------------ #
...
#-------------------- Problem 4 determine relation -------------#
	def determine_relation(self, person, relative):
		person = self.relation_map[person]['self']
		relative = self.relation_map[relative]['self']
		if relative == self.get_mother(person):
			return 'mother'
		elif relative == self.get_father(person):
			return 'father'
		elif relative in self.get_sons(person):
			return 'son'
		elif relative in self.get_daughters(person):
			return 'daughter'
		elif relative in self.get_grandsons(person):
			return 'grandson'
		elif relative in self.get_granddaughters(person):
			return 'granddaughter'
		elif relative in self.get_brothers(person):
			return 'brother'
		elif relative in self.get_sisters(person):
			return 'sister'
		elif relative in self.get_cousins(person):
			return 'cousin'
		elif relative in self.get_sister_in_laws(person):
			return 'sister-in-law'
		elif relative in self.get_brother_in_laws(person):
			return 'brother-in-law'
		elif relative in self.get_paternal_uncle(person):
			return 'paternal-uncle'
		elif relative in self.get_paternal_aunt(person):
			return 'paternal-aunt'
		elif relative in self.get_maternal_uncle(person):
			return 'maternal-uncle'
		elif relative in self.get_maternal_aunt(person):
...
def fillFamilyTree():
	...
	fTree.add_child(father="Satya", son="Saayan")
	...
	fTree.add_couple(husband="Saayan", wife="Mina")
	### End Third level
	...
	fTree.add_child(father="Saayan", son="Misa")
...
if __name__ == '__main__':
	...
	#4. (WHO'S YOUR DADDY)
	print(fTree.determine_relation(person="Kriya", relative="Saayan"))
	# End 4.
################